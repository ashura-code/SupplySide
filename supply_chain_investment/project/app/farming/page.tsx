...
const Farming = async () => {
  ...
  async function getData() {
    const res = await fetch('http://localhost:1337/api/farmings');
  ...
  console.log(Cards[2].attributes);
      ...
      <div className={styles['farming']}>
        {Cards?.length > 0 ? (
          <div className={styles['cards-container']}>
            {Cards.map((card: { attributes: { key: Key | null | undefined; img: string; title: string; raised: number; }; }) => (
               <a href={externalURL} key={1} target="_blank" rel="noopener noreferrer">
                <Block 
